...
void simulate_player(float *p, float *dp, float ddp, float delta_time)
{
    ddp -= *dp * 10.f;

    *p = *p + *dp * delta_time + ddp * delta_time * delta_time * .5f;
    *dp = *dp + ddp * delta_time;

    if (*p + player_half_size_y > arena_half_size_y) {
        *p = arena_half_size_y - player_half_size_y;
        *dp = 0;
    } else if (*p - player_half_size_y < -45) {
        *p = -arena_half_size_y + player_half_size_y;
        *dp = 0;
    }
}
...
void simulate_game(Input* input, float delta_time)
{
    ...
    simulate_player(&player_1_p, &player_1_dp, player_1_ddp, delta_time);
    simulate_player(&player_2_p, &player_2_dp, player_2_ddp, delta_time);
